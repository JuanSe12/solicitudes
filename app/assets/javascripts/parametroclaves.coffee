...
$(document).on 'page:change', ->  #cuando cargue de pagina cargue todo lo que hay en el documento
    ...
    traer_maxlongitud_minlongitud = (id) -> #nos trae la cantidad y valor del producto ,,con el id se sabe cual
        $.ajax   #llamada a ajax
            url: "/parametroclaves/traer_parametroclaves"   #traer_producto  seria el metod en el controlador
            dataType: 'json'           #tipo de dato es un json   
            type: 'post'               #atravez del metodo post
            data: parametroclafe_id: id      #creamos una vble que va hacer igual al id (parametro que se recibe id de productos)
	...
	$('#paramclav_parametroclafe_id').change ->              #cuando cambie de opcion en el select se ejecuta este evento
        if @value
    ...
    $('input#paramclav_maxlongitud').keyup ->  #traigo el id sell_cantidad y keyup captura todo cuando quitamos el dedo de la tecla      
        calcular_total()